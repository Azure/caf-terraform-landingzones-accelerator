resource "azurerm_role_assignment" "kubelet_ingressmsi_miop" {
  ...
  principal_id         = try(module.caf.managed_identities[var.aks_clusters[each.key].kubelet_identity.managed_identity_key].principal_id, each.value.kubelet_identity[0].object_id)
}
...
resource "azurerm_role_assignment" "kubelet_vnet_networkcontrib" {
  ...
  principal_id         = try(module.caf.managed_identities[var.aks_clusters[each.key].identity.managed_identity_key].principal_id, each.value.identity[0].principal_id)
}
