...
git pull
git checkout {{ asvm_connectivity.gitops.landingzones }}
...
{% endif %}
  ...
  -var-folder {{ config_asvm.configuration_folders['asvm'].destination_base_path }}/{{ config_asvm.configuration_folders['asvm'].destination_relative_path }}/{{ level }}/{{ asvm_folder }}/connectivity \
  ...
  -level {{ level }} \
  -p ${TF_DATA_DIR}/{{ tfstates_asvm[asvm_folder].connectivity.tfstate }}.tfplan \
  -a plan
